...
│
├── src/
│   ├── bge.py                               # BGEExtractor: 리뷰 CLS + 패치 청크 mean-pool
...
*No Man's Sky*의 Steam 리뷰와 14개 공식 패치노트를 매칭한 데이터셋을 사용한다. 리뷰는 `txt_file_name` 컬럼으로 그 시점의 패치노트와 연결되며, `has_patch_notes == True`인 행만 학습에 사용된다. 모델이 실제로 사용하는 주요 컬럼은 다음과 같다.

| 컬럼 | 역할 | 설명 |
|---|---|---|
| `review` | 입력 (텍스트) | 리뷰 본문 |
| `votes_up` | 타깃 | 리뷰 유용성 (추천 수) → `log1p` 변환 후 예측 |
...
<table>
  ...
  <tbody>
    <tr><td>TNN</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>
    <tr><td>HP-BERT</td><td>0.664</td><td>0.974</td><td>0.987</td><td>34.7953</td></tr>
    <tr><td>DMMN</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>
    <tr><td><b>PRIS-RHP</b></td><td><b>0.369</b></td><td><b>0.399</b></td><td><b>0.631</b></td><td><b>31.317</b></td></tr>
  </tbody>
</table>

<p align="center">
  <img src="model/result_plot.png" alt="PRIS-RHP 실험 결과" width="700">
</p>

## 문의 (Contact)

질문, 협업, 피드백은 아래로 연락주시기 바랍니다.
**(박종화)**
(빅데이터공학)
Email: [your_email@hansung.ac.kr](mailto:your_email@hansung.ac.kr)
